...
- **Document upload:** Securely upload encrypted files directly from cases or upload portal. (IMPLEMENTED)
...
- **Document search:** Fully integrated RAG semantic search across documents. (IMPLEMENTED)
- **Document preview:** Viewing documents securely. (IMPLEMENTED)
- **Secure sharing:** Access request workflow. (PLANNED)
- **Evidence management:** Track evidence metadata. (IMPLEMENTED)
- **Chain of custody:** Track every action on a document in `EvidenceChain` and Hyperledger simulation. (IMPLEMENTED)
- **Audit trail:** Access audit logs visually displayed on individual documents. (IMPLEMENTED)
- **Digital signatures:** Mock HMAC-based signatures. (IMPLEMENTED)
- **Hash/integrity verification:** Real SHA-256 hash generation on upload. (IMPLEMENTED)
- **Encryption:** AES-256 file encryption prior to simulated S3 storage. (IMPLEMENTED)
- **Blockchain verification:** Simulated Hyperledger Fabric anchoring in database upon upload. (IMPLEMENTED)
- **AI/OCR:** Simulated LangChain Vector Embeddings and PaddleOCR via FastAPI microservice. (IMPLEMENTED)
- **Notifications:** In-app user notifications. (PLANNED)
- **Dashboard:** Overview of system activity and security. (IMPLEMENTED)
